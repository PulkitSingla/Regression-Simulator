...
app.layout = html.Div(html.Div(children=[
    ...
    html.Div([
        html.Div([html.Div([html.Label('Select Regression Model:', className='h5'),
                            dcc.Dropdown(id='model-dropdown',
                                         options=[
                                             ...
                                             {'label': 'RANSAC Linear Regression',
                                              'value': 3},
                                             {'label': 'Theilsen Linear Regression',
                                              'value': 4},
                  ...
                  html.Div(id='svr-inputs', children=[
                      ...
                      html.Div([
                              html.Div([
                                  html.Label(
                                      'Penalty Ratio, C:', className='col-6'),
                                  dcc.Input(
                                      id='cost-function', type='number', step=0.01, placeholder='C=1.0', className='col-6')
                              ], className='col-12 p-0 mt-1'),
                          html.Div([
                              html.Label(
                                  'Epsilon for Support-Vector:', className='col-6'),
                              dcc.Input(
                                  id='epsilon', type='number', step=0.01, placeholder='epsilon=0.5', className='col-6')
                          ], className='col-12 p-0 mt-1'),
                          dcc.Checklist(id='hide-svr-tolerance', options=[
                              {'label': ' Show SVR Tolerance', 'value': 1}], value=[1], className='col-12 mt-1')
        ...
        # Block Below Graph
        html.Div([
            html.Label('Select Data and Regression Columns:',
                       className='h5 col-12'),
            ...
            html.Div([html.Div(dcc.Upload(id='upload-data',
                      ...
                      html.Div([
                          html.Div([
                              html.Label(
                                  id='row-trimmer-label', children='Trim Training Data:', className='col-12'),
                              dcc.Slider(id='row-trimmer', min=0.01, max=1, value=1, step=0.01,
                                         marks={}, updatemode='mouseup', className='col-12 p-0')
                          ], className='col-7'),
                          html.Div([
                              html.Label(
                                  'Split Train/Test:', className='col-6 text-center p-0 m-0'),
                              daq.BooleanSwitch(
                                  id='train-test-split', on=False, className='col-6 p-0 m-0'),
                          ], className='col-5')
                      ], className='row mb-2'),
                      ...
                      html.Div([html.Label(dcc.Markdown('*__OR__ Select Built-in Sample Data:*')),
                                html.Div(dcc.Dropdown(id='select-dataset',
                                                      options=[
                                                          {'label': 'No Outliers',
                                                           'value': 'df_normal'},
                                                          {'label': 'Small Outliers in x',
                                                           'value': 'df_x_errors'},
                                                          {'label': 'Small Outliers in y',
                                                           'value': 'df_y_errors'},
                                                          {'label': 'Large Outliers in x',
                                                           'value': 'df_x_large_errors'},
                                                          {'label': 'Large Outliers in y',
                                                           'value': 'df_y_large_errors'},
                                                          {'label': 'Sine Function with Noises & Outliers',
                                                           'value': 'df_rbf_eps'},
                                                          {'label': 'Sine Function',
                                                           'value': 'df_rbf_c'}
                                                      ],
...
def regression_model(kernel_selected, cost_function, epsilon, json_intermediate_data, x_column, y_column, model, svr_tolerance, default_dataset, rows_selected):
    ...
    if x_column is None and y_column is None:
        ...
        # Prepare Test data ---
        X_test = test_df.sort_values(
            x_column)[x_column].as_matrix().reshape(-1, 1)
        ...
        if 1 in model:
            ...
            if rows_selected < 1:
                name_test = "OLS Reg., R-sq={:.2%}".format(
        ...
        if 2 in model:
            for kernels in kernel_selected:
                svr = SVR(kernel=kernels, C=float(
                    cost_function), epsilon=float(epsilon))
                svr_model = svr.fit(X, y)
                y_predicted = svr_model.predict(X)
                name = "Support-Vector Reg. " + \
                    kernels.upper() + ", R-sq: {:.2%}".format(svr.score(X, y))

                if 1 in svr_tolerance:
                    traces.append(go.Scatter(
                        x=X.ravel().tolist() + X.ravel().tolist()[::-1],
                        y=[y+float(epsilon) for y in y_predicted.tolist()] +
                        [y-float(epsilon) for y in y_predicted.tolist()][::-1],
                ...
                if rows_selected < 1:
                    name_test = "Support-Vector Reg. " + \
                        kernels.upper() + \
                        ", R-sq: {:.2%}".format(svr.score(X_test, y_test))
                    traces_test.append(go.Scatter(
                        x=X_test.ravel(),
        ...
        if 3 in model:
            ...
            name = "RANSAC Reg., R-sq={:.2%}".format(
                ransac.score(X[inlier_mask_ransac], y[inlier_mask_ransac]))
            ...
            if rows_selected < 1:
                name_test = "RANSAC Reg., R-sq={:.2%}".format(
        ...
        if 4 in model:
            ...
            if rows_selected < 1:
                name_test = "Theilsen Reg., R-sq={:.2%}".format(
        ...
        if 5 in model:
            ...
            if rows_selected < 1:
                name_test = "Huber Reg., R-sq={:.2%}".format(
        ...
        return {
            'data': traces,
            'layout': go.Layout(
                title="Regression Modeling on Single Variable (Train: {:.1%})".format(
                    rows_selected),
            ...
            'data': traces_test,
            'layout': go.Layout(
                title="Regression Modeling on Single Variable (Test: {:.1%})".format(
                    1-rows_selected),
                font=dict(family='sans-serif', size=10, color='#7f7f7f'),
                xaxis={'title': x_column},
                yaxis={'title': y_column},
